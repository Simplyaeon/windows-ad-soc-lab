...
| 04 | [Event Viewer & the Logging Model](./labs/04-event-viewer-logging-model.md) | 1102, 104, 4719, 4688 | 🟡 Written, not yet run |
...
| 2026-08-24 | 04 — Event Viewer & Logging Model | 🟡 Written, not yet run. Six steps, mostly Event Viewer clicks: the log tree and which VM writes what, an event's General vs XML view, building filters by clicking and saving three custom views, measuring real log retention, then destroying evidence two ways — silent overwrite (shrink + 4688 flood) and an outright clear (1102 / 104). Built around one skill: telling "nothing happened" apart from "I can't see it." Module 01's open Finding 2 is answered by custom View 2. |

...
| 03 | [The Windows Registry & Persistence](./labs/03-windows-registry.md) | 4657, Sysmon 12/13/14 | ✅ Complete |
...
| 2026-09-10 | 03 — The Windows Registry & Persistence | 🟡 **Written, not yet run.** WS01-only run sheet built around one contrast: **native registry auditing is opt-in per key, Sysmon is opt-out per rule.** Tour the hives as a filesystem (`Get-ChildItem` = subkeys, `Get-ItemProperty` = values), baseline the `Run` keys, then enable the **`Registry`** subcategory and SACL `HKLM\…\Run` — Module 02's four gates again, with **Set Value** as the gate-4 right that `WRITE_DAC` was there. Plant benign `notepad.exe` persistence in **three** places (HKLM `Run` via `reg add`, HKCU `Run` via `Set-ItemProperty` — **no admin needed** — and HKLM `RunOnce`), of which only one is under a SACL. **4657** catches one and carries the `OldValue`/`NewValue` pair nothing else in the Security log gives you; the other two are invisible, and proving that absence properly is the module's real finding. Then install Sysmon (bidirectional drag-and-drop from the host, no internet), write a five-rule `onmatch="include"` config by hand, replant, and catch all three as **Event 13**. Deliverable: a top-10 persistence table with the hunt for each. |
| 2026-09-12 | 03 — The Windows Registry & Persistence | ✅ **Complete.** Run across two sittings on WS01. The module's thesis proved itself inside a single eight-minute window: three identical `notepad.exe` persistence writes (`HKLM\…\Run` at **21:00:22 UTC** via `reg.exe`, `HKCU\…\Run` at **21:06:25** via `Set-ItemProperty` with **no elevation**, `HKLM\…\RunOnce` at **21:08:15** via `reg.exe`), both instruments live — **native 4657 returned one of the three; Sysmon Event 13 returned all three.** The same query that missed two *returned* the third, so the query, window, log health and host are excluded as explanations by its own positive result. **Finding 2**: a coverage failure, not a configuration failure — auditing worked perfectly for the one key it was pointed at, and the two misses include the only write needing no privileges and the only one that self-deletes. **Finding 1** came out stronger than designed: Sysmon **Event 1** caught `reg.exe` **63 ms** before its Event 13, carrying the full command line, `ParentImage`, `User` and `IntegrityLevel` — "a value changed" became "this account ran this command." Four corrections the run forced into the lab file: Sysmon logs the per-user hive as **`HKU\<SID>\`, never `HKCU\`** (the mirror of 4657's `\REGISTRY\MACHINE\`, and a hunt string built from either shorthand returns nothing); a deleted *value* is an **Event 12 `DeleteValue`**, not a 13, so a 13-only rule sees arrival and never cleanup; Sysmon's **bare defaults log no registry events at all**, so the config *adds* visibility rather than trimming a flood; and `Details` preserves case, so value-data hunts must be case-insensitive. Also observed and recorded honestly as unexplained: `sihost.exe` writes a matching `RunNotification\StartupTNoti<name>` entry for every `Run` value (none for `RunOnce`) on a variable delay, and `ParentImage` comes back empty for Store-packaged apps. The 2026-09-10 anomaly stays parked — still no cause. Sysmon v15.15 left installed and configured for Modules 06–07. |
